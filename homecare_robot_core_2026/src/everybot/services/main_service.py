...
class MainService:
    ...
    def tick(self) -> None:
        ...
        if (self._ctx.reg_state == RegState.REGISTED or self._ctx.reg_state == RegState.DONE) and self.mqtt_cfg.enabled and not self._mqtt.started:
            ha_cfg = None
            if self.ha_enabled and self._ctx.smarthome_ip:
                ha_cfg = HomeAssistantConfig(
                    ha_ip=self._ctx.smarthome_ip,
                    token=self._ctx.smarthome_token or "",
                    timeout_sec=3.0,
                )
            self._mqtt.start(ha_cfg, self._state_store, self.projector)
    ...
    def _handle_wired(self, msg: dict) -> None:
        ...
        if (requestType == "request_iot_control"):
            if not self.ha_enabled:
                log.debug("[main] iot_control skipped — ha disabled")
            elif self._mqtt.started:
        ...
        if (requestType == "request_get_iot_device_info"):
            if not self.ha_enabled:
                # HA 비활성: 빈 device 목록 응답 후 즉시 반환
                self.wired._send_obj({"type": "report_device_info", "payload": {}})
            else:
                try:
                    if self._mqtt.started:
                        devices = self._mqtt._build_save_iot_devices_payload()
