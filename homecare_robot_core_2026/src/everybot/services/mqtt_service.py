...
class MqttService:
    ...
    def __post_init__(self) -> None:
        ...
        self._ha_unavailable_until: float = 0.0   # HA 연결 실패 시 cooldown 시각
    ...
    def start(self, haCfg: HomeAssistantConfig | None, stateStore: StateStore, projectorUtil: BeamProjectorUtil) -> None:
    ...
    def _build_save_iot_devices_payload(self) -> dict:
        # HA 연결 실패 후 cooldown: 60초간 재시도 억제 (블로킹 방지)
        if time.monotonic() < self._ha_unavailable_until:
            log.debug("[mqtt][iot] HA cooldown active — returning empty")
            return {}

        self._ensure_ha_client()
        if getattr(self, "_ha", None) is None:
            log.debug("[mqtt][iot] HA client not configured — returning empty")
            return {}

        states = self._ha.list_states()
        if states is None:
            # HA 연결 실패 → 60초 cooldown 설정
            self._ha_unavailable_until = time.monotonic() + 60.0
            log.warning("[mqtt][iot] HA unreachable — cooldown 60s")
            return {}
        states = states or []
