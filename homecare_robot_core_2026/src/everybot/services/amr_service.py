...
class AmrService:
    ...
    def __post_init__(self) -> None:
        ...
        self._arrived_state: int | None = None
    ...
    def pop_arrived_event(self) -> bool:
        """엣지 트리거: 도착 상태 전환 순간 1회만 True 반환."""
        with self._lock:
    ...
    @property
    def cached_arrived_state(self) -> int | None:
        """마지막 도착 이벤트가 발생한 movingState."""
        with self._lock:
            return self._arrived_state
    ...
    def send_nav_cmd(self, coord: dict) -> None:
        """목적지 설정(cmd=60) + 주행 시작(cmd=61) 전송.

        AMR 프로토콜: type 필드가 있는 TargetPosition 은 cmd=61 START 를
        명시 전송해야 실제 주행이 시작된다.
        """
        x, y, theta = (coord.get(k, 0.0) for k in ("x", "y", "theta"))
        body = build_nav_request(x, y, theta)
        self._send(AmrCmd.TARGET_POSITION, AmrDtype.SET, body)
        # cmd=61 Driving START — 목적지 설정만으로는 주행 시작 안 됨
        self._send(AmrCmd.DRIVING, AmrDtype.SET,
                   build_driving_request(DrivingSet.START))
        log.info("[AMR] send_nav_cmd → (%.2f, %.2f, %.2f) + driving_start", x, y, theta)
                ...
                if resp_api:
                    ...
                    if resp_api_key == 'AllMovingInfo':
                        ...
                        with self._lock:
                            ...
                            # 도착 감지: MOVING→ARRIVED(2) 또는 MOVING→ALTERNATIVE_GOAL(8)
                            # MOVING→IDLE 은 정지/취소도 포함하므로 도착으로 보지 않는다.
                            if prev == MovingState.MOVING and state in (
                                MovingState.ARRIVED, MovingState.ALTERNATIVE_GOAL
                            ):
                                self._arrived_flag = True
                                self._arrived_state = state
                                label = (
                                    "ALTERNATIVE_GOAL"
                                    if state == MovingState.ALTERNATIVE_GOAL
                                    else "ARRIVED"
                                )
