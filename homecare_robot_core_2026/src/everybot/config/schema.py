...
class FeatureSettingsConfig:
    """5/21 데모 기능별 통합 설정 파일 경로."""
    file_path: str = "configs/feature_settings.json"


@dataclass(frozen=True)
class ProgramScheduleConfig:
    """모닝콜에서 안내할 프로그램 일정 파일 경로."""
    file_path: str = "configs/program_schedule.json"
...
@dataclass(frozen=True)
class HaConfig:
    """HomeAssistant IoT 연동 on/off. ha_ip/token은 state.json에서 로드."""
    enabled: bool = False
...
class RobotConfig:
    ...
    feature_settings: FeatureSettingsConfig = field(default_factory=FeatureSettingsConfig)
    program_schedule: ProgramScheduleConfig = field(default_factory=ProgramScheduleConfig)
