...
<div id="phase-3" class="phase">
  ...
  <div id="wp-list">
    <table>
      <thead><tr><th>Key</th><th>Label</th><th>X</th><th>Y</th><th>θ</th><th>Type</th><th>Comment</th><th></th></tr></thead>
...
function goPhase(n) {
  ...
  if (n === 3) drawMap(document.getElementById('map3'));
  if (n === 4) {
...
function drawZonePolygon(ctx, canvas, zone, idx, forbidden) {
  ...
  ctx.fillText(zone.key || `zone-${idx + 1}`, lx + 8, ly - 8);
  ctx.restore();
}
...
function addWaypoint() {
  ...
  waypoints.push({ key, label, x, y, theta, type, comment });
  renderWpTable();
  drawMap(document.getElementById('map3'));
  // 폼 초기화
  ['wp-key','wp-label'].forEach(id => document.getElementById(id).value = '');
}
...
function renderWpTable() {
  ...
  waypoints.forEach((wp, i) => {
    ...
    tr.innerHTML = `<td>${wp.key}</td><td>${wp.label}</td>
      ...
      <td>${wp.theta.toFixed(2)}</td><td>${wp.type}</td><td>${wp.comment}</td>
      <td><button class="del-btn" onclick="delWp(${i})">삭제</button></td>`;
...
function renderZoneTable(mode) {
  ...
  zones.forEach((zone, i) => {
    ...
    tr.innerHTML = `<td>${zone.key || ''}</td><td>${zone.label || ''}</td>
      <td>${pointCount}</td>
      <td><button class="del-btn" onclick="deleteZone('${mode}', ${i})">삭제</button></td>`;
    tbody.appendChild(tr);
  });
}
...
class _Handler(BaseHTTPRequestHandler):
    ...
    def _handle_nav_test(self, body: dict) -> None:
        ...
        # send_nav_cmd() 는 type=2 + cmd=61 즉시 전송으로 AMR 미동작 확인됨.
