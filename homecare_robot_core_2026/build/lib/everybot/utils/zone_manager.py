...
class ZoneManager:
    ...
    def get(self, key: str) -> dict | None:
        ...
        return next(
            (
                ...
                if zone.get("key") == key or zone.get("zone_id") == key
            ),
            None,
        )
    ...
    def get_as_bypass_areas(self) -> list[dict]:
        ...
        return [
            {
                "id": zone.get("key", zone.get("zone_id", "")),
    ...
    def find_zones_containing(self, x: float, y: float) -> list[str]:
        ...
        return [
            str(zone.get("key", zone.get("zone_id", "")))
            for zone in self._zones
            if _ray_cast(x, y, zone.get("polygon", []))
        ]

    def get_zone(self, x: float, y: float) -> dict | None:
        """좌표가 속한 첫 번째 ROI zone을 표준 스키마로 반환."""
        for zone in self._zones:
            if _ray_cast(x, y, zone.get("polygon", [])):
                zone_id = str(zone.get("zone_id", zone.get("key", "")) or "")
                zone_name = str(zone.get("zone_name", zone.get("label", "")) or "")
                return {
                    "zone_id": zone_id,
                    "zone_name": zone_name,
                    "key": zone_id,
