...
from .schema import (
    ...
    RobotSettingsConfig, TtsConfig, MapCreationConfig,
...
def load_robot_config(path: Path) -> RobotConfig:
    ...
    return RobotConfig(
        ...
        ui_mqtt=UiMqttConfig(
            ...
            topic_cmd=str(ui_mqtt.get("topic_cmd", "ui/setup_cmd")),
            topic_event=str(ui_mqtt.get("topic_event", "ui/human_interactive_event")),
            topic_status=str(ui_mqtt.get("topic_status", "ui/status")),
            subscribe_topics=tuple(ui_mqtt.get(
                "subscribe_topics",
                ["/ui/event", "/ui/call", "/ui/setting/change", "/ui/picture_capture"],
            )),
            legacy_subscribe_topics=tuple(ui_mqtt.get("legacy_subscribe_topics", ["ui/setup_cmd"])),
            topic_robot_status=str(ui_mqtt.get("topic_robot_status", "/robot/status")),
            topic_robot_change_ui=str(ui_mqtt.get("topic_robot_change_ui", "/robot/change_ui")),
            topic_robot_heartbeat=str(ui_mqtt.get("topic_robot_heartbeat", "/robot/heartbeat")),
            topic_robot_setting_now=str(ui_mqtt.get("topic_robot_setting_now", "/robot/setting/now")),
            topic_robot_event=str(ui_mqtt.get("topic_robot_event", "/robot/event")),
            status_types=tuple(ui_mqtt.get("status_types", ["robot_status", "request_show_menu", "REG_STATUS"])),
            event_type_prefixes=tuple(ui_mqtt.get("event_type_prefixes", ["notify_", "response_"])),
        ),
        map=MapConfig(
